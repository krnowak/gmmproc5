...
{
  m_token_types_actions[ "include" ] = std::make_pair( INCLUDE, on_include );
  m_token_types_actions[ "define-flags-extended" ] = std::make_pair( FLAGS, on_flags );
  m_token_types_actions[ "define-enum-extended" ] = std::make_pair( ENUM, on_enum );
  m_token_types_actions[ "define-object" ] = std::make_pair( OBJECT, on_object );
  m_token_types_actions[ "define-function" ] = std::make_pair( FUNCTION, on_function );
  m_token_types_actions[ "define-method" ] = std::make_pair( METHOD, on_method );
  m_token_types_actions[ "define-property" ] = std::make_pair( PROPERTY, on_property );
  m_token_types_actions[ "define-signal" ] = std::make_pair( SIGNAL, on_signal );
  m_token_types_actions[ "define-vfunc" ] = std::make_pair( VFUNC, on_vfunc );
  m_token_types_actions[ "define-flags" ] = std::make_pair( OMIT, on_omit );
  m_token_types_actions[ "define-enum" ] = std::make_pair( OMIT, on_omit );
}
...
void DefsParser::parse_statement( const std::string& statement )
{
  size_t      space_idx ( 0 );
  std::string copy      ( statement );
  std::string type;
  std::map< std::string, std::pair< DefineType, void (*DefsParser)( const std::string& statement ) > >::iterator it;
  
  copy.erase( copy.find_last_of( ")" ) );
  copy.erase( 0, copy.find_first_of( "(" ) + 1 );
  space_idx = copy.find_first_of( " " );
  type = copy.substr( 0, space_idx );
  it = m_token_types_actions.find( type );
...
void DefsParser::create_namespace( DefineType type, const std::string& statement )
{
...
int main( int argc, char** argv )
{
  if( argc < 2 )
  {
    std::cerr << "got no param" << std::endl;
    return 1;
  }
  else
  {
    Proc::DefsParser parser( argv[1] );
    parser.parse();
  }
  return 0;
}
